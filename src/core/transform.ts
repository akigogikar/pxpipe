...
export async function transformRequest(
  ...
  if (o.compressTools && Array.isArray(req.tools) && req.tools.length > 0) {
    ...
    toolsRewritten = req.tools.map((t) => {
      ...
      // "tools.N.<type>.description: Extra inputs are not permitted"). Only the
      // default 'custom' shape (no `type`, or `type: 'custom'`) accepts a
      // description/input_schema rewrite. Pass anything else through byte-identical:
      // it never had docs to relocate, so there's nothing to image either.
