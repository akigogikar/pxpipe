...
/** Imaged doc (heading + compact schema JSON) for one tool. On the GPT path the
 *  native tools[] KEEP their `description` — only the schema *annotations* are
 *  stripped (see rewriteToolsForGpt). So the description must NOT be imaged here:
 *  doing so double-bills it (native text + image pixels) while the savings baseline
 *  (gptBaselineImagedTokens) credits only the stripped-schema delta. We image the
 *  heading + full schema (which carries the stripped annotations at image rates);
 *  the description rides natively. (The Anthropic path differs on purpose — it stubs
 *  the native description and images the full doc, a real move, not a duplicate.) */
